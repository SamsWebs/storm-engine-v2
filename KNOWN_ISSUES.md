...
- ~~**`ContactSystem`'s broadphase sweeps one axis.**~~ **Fixed.** It was a sort on `minX` with an early break, which degraded to all-pairs for anything stacked in a single column. It is a uniform grid now, with no preferred axis; `SetCellSize` overrides the size derived per frame from the bodies present.
