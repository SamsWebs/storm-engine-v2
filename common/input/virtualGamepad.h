...
// in a SNES-style diamond bottom-right (X top, Y left, A right, B bottom).
//
// Pure logic: MakeVPadLayout(w, h) positions everything from the window size and
// EvalVPad(layout, fingers) maps touches onto the button/direction flags. The
// game supplies fingers in its logical coordinate space and decides what each
// button does; the render layer draws the zones from the same layout.
