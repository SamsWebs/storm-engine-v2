...
bool NetSnapshot::AddItem(uint16_t type, uint16_t id, const int32_t *data, int count) {
    ...
    if (FindKey(key, index)) {
        // Replace in place; counts must match so offsets stay stable.
        if (offsets_[index] + count > dataCount_ || count < 0)
            return false;
...
int NetSnapshot::Finish() {
    ...
    for (int i = 0; i < numItems_; i++)
        order_[i] = i;
    std::sort(order_, order_ + numItems_, [this](int a, int b) {
...
bool NetSnapshot::FindItem(uint16_t type, uint16_t id, const int32_t *&data, int &count) const {
    if (!finished_)
        return false;
    uint32_t key = MakeKey(type, id);
...
int NetSnapshotDelta::Create(const NetSnapshot &from, const NetSnapshot &to,
    ...
    for (int i = 0; i < to.NumItems() && written >= 0; i++) {
        ...
        putVarInt(tc);
        for (int j = 0; j < tc; j++)
            putVarInt(inFrom ? td[j] - fd[j] : td[j]);
    }

    return written;
}

bool NetSnapshotDelta::Apply(const NetSnapshot &from, const uint8_t *delta, int deltaSize,
                             NetSnapshot &to) {
    to.Reset();
    int pos = 0;
    ...
    if (!getVarInt(numUpdated) || numUpdated < 0 || numUpdated > NetSnapshot::kMaxItems)
        return false;

    // Copy the base items, skipping deleted keys.
    for (int i = 0; i < from.NumItems(); i++) {
        uint16_t t, id;
        const int32_t *d;
        int c;
        ...
        if (!deleted && !to.AddItem(t, id, d, c))
            return false;
    }

    // Apply updates: diffs against the base, absolutes for new items.
    for (int u = 0; u < numUpdated; u++) {
        ...
        if (count > NetSnapshot::kMaxDataInts)
            return false;
        for (int j = 0; j < count; j++) {
            int32_t v = 0;
            if (!getVarInt(v))
                return false;
            values[j] = inFrom ? fd[j] + v : v;
        }
...
int NetSnapshotDelta::EstimateSize(const NetSnapshot &from, const NetSnapshot &to) {
