...
public:
  ...
  void Reset();
  bool AddItem(uint16_t type, uint16_t id, const int32_t *data,
               int count); // replaces on duplicate key
  ...
  bool FindItem(uint16_t type, uint16_t id, const int32_t *&data,
                int &count) const;
