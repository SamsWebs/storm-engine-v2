...
void AnimationSystem::Update() {
  for (const auto &entity : GetSystemEntities()) {
    ...
    animation.mCurrentFrame = ((SDL_GetTicks() - animation.mStartTime) *
                               animation.mFrameSpeedRate / 1000) %
                              animation.mNumFrames;
    ...
    if (animation.mVertical) {
      ...
      sprite.mSrcRect.x =
          (animation.mCurrentFrame * sprite.mWidth) + animation.mFrameOffset;
    }
  }
}
