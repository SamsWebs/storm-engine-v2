...
void ImGuiFuncs::TileSetWindow(const AssetManager_Ptr &assetManager,
                               const glm::vec2 &mouseRect) {
  //
  if (ImGui::Begin("Texture", &mImageLoaded,
                   ImGuiWindowFlags_HorizontalScrollbar)) // We also want the
                                                          // horizontal bar
  {
    // The image might be larger than the ImGui window, so we want to add the
    // current scroll value to the mouse position for the selected image
    ...
    ImGui::Image(assetManager->GetTexture(mAssetID).get(),
                 ImVec2(imageWidth, imageHeight));

    int mousePosX = static_cast<int>(ImGui::GetMousePos().x -
                                     ImGui::GetWindowPos().x + scrollX);
    int mousePosY =
        static_cast<int>(ImGui::GetMousePos().y - ImGui::GetWindowPos().y -
                         TITLE_BAR_SIZE + scrollY);
...
void ImGuiFuncs::UpdateShortCuts(
    sol::state &lua, const AssetManager_Ptr &assetManager, Renderer &renderer,
  ...
  if ((state[SDL_SCANCODE_LCTRL] || state[SDL_SCANCODE_RCTRL]) &&
    ...
    Save(assetManager, renderer, canvas->GetWidth(), canvas->GetHeight(),
         tileSize);
  }

  // OpenFile Dialog
  if ((state[SDL_SCANCODE_LCTRL] || state[SDL_SCANCODE_RCTRL]) &&
      state[SDL_SCANCODE_O]) {
    OpenProject(lua, assetManager, renderer, canvas, tileSize);
  }

  // Call to create new canvas
  if ((state[SDL_SCANCODE_LCTRL] || state[SDL_SCANCODE_RCTRL]) &&
      state[SDL_SCANCODE_N]) {
    mNewFile = true;
  }
...
void ImGuiFuncs::OpenProject(sol::state &lua,
  ...
  mFileLoader->LoadProject(lua, mFileName, assetManager, renderer,
                           mLoadedTilesets, mTilesetLocations, canvas,
                           tileSize);
  SetWindowName(mFileName);
}

void ImGuiFuncs::Save(const AssetManager_Ptr &assetManager, Renderer &renderer,
...
ImGuiFuncs::ImGuiFuncs(std::shared_ptr<MouseControl> &mouseControl)
    : mFileName(""), mImageName(""), mAssetID(""),
      mWindowName("Tilemap Editor"), mLuaTableFile(""), mTileProps(),
      mPrevTileProps(), mWidth(16), mHeight(16), mImageWidth(0),
      mImageHeight(0), mImageLoaded(false), mExit(false), mCollider(false),
      mAnimated(false), mCleared(false), mCheck(false), mNewFile(false),
      mUndone(false), mRedone(false), mLoadedTilesets(), mTilesetLocations(),
...
void ImGuiFuncs::ShowFileMenu(sol::state &lua,
                              const AssetManager_Ptr &assetManager,
                              Renderer &renderer,
                              std::shared_ptr<Canvas> &canvas, int &tileSize) {
  ...
  if (ImGui::MenuItem(ICON_FA_SAVE " Save As..", "Ctrl + Shift + S")) {
    ...
    mFileLoader->SaveProject(mFileName, mLoadedTilesets, mTilesetLocations,
                             canvas->GetWidth(), canvas->GetHeight(), tileSize);
  }
  ...
  if (ImGui::MenuItem(ICON_FA_SAVE " Save To Lua Table")) {
    ...
    mFileLoader->SaveToLuaTable(mLuaTableFile, mLoadedTilesets,
                                mTilesetLocations, tileSize);
  }
...
void ImGuiFuncs::ShowToolsMenu(Renderer &renderer,
                               const AssetManager_Ptr &assetManager) {
  if (ImGui::MenuItem(ICON_FA_PLUS " Add Tileset")) {
    FileDialogWin fileDialog;
    mImageName = fileDialog.OpenImageFile();

    // If the action was cancelled or returned an empty string, exit the
    // function
    if (mImageName == std::string() || mImageName == "")
      return;

    // Create a filesystem path so we can get the stem and save that as the
    // AssetID for later usage
    ...
    // We need to query the texture to get the image width/height. This is used
    // for setting the src_Rect positions
...
void ImGuiFuncs::ShowTileProperties(std::shared_ptr<MouseControl> &mouseControl,
                                    const AssetManager_Ptr &assetManager,
                                    bool collider) {
  ...
  if (ImGui::Begin(boxName.c_str())) {
    if (!collider) {
      ...
      if (currentTileset != "") {
        if (prevTileSet != currentTileset) {
          ...
          SDL_QueryTexture(assetManager->GetTexture(mAssetID).get(), NULL, NULL,
    ...
    if (!collider)
      mouseControl->SetSpriteProperties(mAssetID, mWidth, mHeight,
                                        mTileProps.layer, mTileProps.srcRectX,
                                        mTileProps.srcRectY);
    ...
    if (mCollider && CheckCollidersStatus())
      mouseControl->SetBoxColliderProperties(
          mTileProps.boxWidth, mTileProps.boxHeight, mTileProps.boxOffsetX,
          mTileProps.boxOffsetY);

    if (mAnimated && CheckAnimationStatus())
      mouseControl->SetAnimationProperties(
          mTileProps.numFrames, mTileProps.frameSpeed, mTileProps.vertical,
          mTileProps.looped, mTileProps.srcRectX);
