...
private:
  struct TileProperties {
    int scaleX, scaleY, boxWidth, boxHeight, boxOffsetX, boxOffsetY, layer,
        srcRectX, srcRectY, mouseRectX, mouseRectY;
    int numFrames, frameSpeed, frameOffset;
    bool vertical, looped;

    TileProperties(int scale_x = 1, int scale_y = 1, int box_width = 16,
                   int box_height = 16, int box_offset_x = 0,
                   int box_offset_y = 0, int layer = 0, int src_rect_x = 0,
                   int src_rect_y = 0, int mouse_rect_x = 16,
                   int mouse_rect_y = 16, int num_frames = 1,
                   int frame_speed = 10, int frame_offset = 0,
  ...
  bool mImageLoaded, mExit, mCollider, mAnimated, mCleared, mCheck, mNewFile,
      mUndone, mRedone;
...
private:
   ...
   * @return - Returns true if any of the transform properties are different
   ...
   * @return - Returns true if any of the animation properties are different
   * from the previous tile properties.
   */
...
public:
  ...
  /*
   *	The ShowFileMenu opens up an ImGui menu that displays your
   *load/save/exit buttons
   */
  void ShowFileMenu(sol::state &lua, const AssetManager_Ptr &assetManager,
                    Renderer &renderer, std::shared_ptr<Canvas> &canvas,
                    int &tileSize);

  void ShowToolsMenu(Renderer &renderer, const AssetManager_Ptr &assetManager);

  /*
   *  	This is an ImGui window shows the tile properties if [Create Tiles] is
   *selected. You are able to adjust the properties of the tiles here before
   *placing them
   */
  void ShowTileProperties(std::shared_ptr<class MouseControl> &mouseControl,
                          const AssetManager_Ptr &assetManager, bool collider);

  /*
   *	TileSetWindow is the ImGui window that holds the tileset to parse. Once
   *you load a tileset, all you have to do is left click the mouse on any
   *desired tile and this function will set the src_rect for the tile to the
   * 	selected tile.
   *	@param: AssetManager -> Retrieve the Texture from the asset manager for
   *the ImGui::Image
   *	@param: glm::vec2& mouseRect -> This is used to set the src_rect after
   *selecting a tile
   */
  void TileSetWindow(const AssetManager_Ptr &assetManager,
  ...
  /*
   *	Opens up a pop up window when selecting "NEW". Asks the user if they are
   ...
   *	You are able to call the above commands from this using the shortcut
   *keys
   */
  void
  UpdateShortCuts(sol::state &lua, const AssetManager_Ptr &assetManager,
                  Renderer &renderer, std::shared_ptr<Canvas> &canvas,
                  int &tileSize,
                  const std::unique_ptr<class CommandManager> &commandManager);

  /*
   *	OpenProject - Loads a previously saved project from a lua table project
   *file
   *	@param - sol::state& lua -> used to read the lua table.
   *	@param - AssetManager -> Loads the required assets into the asset
   *manager from the lua file
  ...
  /*
   *  Save - Checks to see if the file has already been save, if not, opens a
   *file dialog and saves the project by creating a new project file, tiles file
   *and colliders file.
   */
  void Save(const AssetManager_Ptr &assetManager, Renderer &renderer,
