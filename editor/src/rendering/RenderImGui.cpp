...
void RenderGuiSystem::Update(const AssetManager_Ptr &assetManager,
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu(ICON_FA_EDIT " Edit")) {
      ...
      if (ImGui::InputInt("Canvas Width", &mCanvasWidth, mTileSize,
                          mTileSize)) {
        // If there is a change of width, add the canvas change to the command
        // stack
      ...
      if (ImGui::InputInt("Canvas Height", &mCanvasHeight, mTileSize,
                          mTileSize)) {
        // If there is a change of height, add the canvas change to the command
        // stack
    ...
    if (ImGui::BeginMenu(
            /*ICON_FA_TOOLS*/ "Tools")) // Tools Icon shows up as question mark
    {
  ...
  if (mCreateTiles) {
    ...
    // If the mouse is off the canvas, do not render the mouse box and do not
    // create a new tile
    if (!MouseOffCanvas())
      mMouseControl->CreateTile(assetManager, renderer, mouseBox, camera,
                                event);

    if (mMouseControl->TileAdded()) {
      mCommandManager->ExecuteCmd(
          std::make_shared<AddTileCommand>(mMouseControl));
    ...
    if (mMouseControl->TileRemoved()) {
      mCommandManager->ExecuteCmd(
          std::make_shared<RemoveTileCommand>(mMouseControl));
  ...
  if (mCreateColliders) {
    ...
    if (!MouseOffCanvas())
      mMouseControl->CreateCollider(assetManager, renderer, mouseBox, camera,
                                    event);
  }
  ...
  mImFuncs->UpdateShortCuts(mLua, assetManager, renderer, mCanvas, mTileSize,
                            mCommandManager);
  UpdateCanvas();
}

void RenderGuiSystem::RenderGrid(Renderer &renderer, SDL_Rect &camera,
                                 const float &zoom) {
  ...
  for (int i = 0; i < yTiles; i++) {
    for (int j = 0; j < xTiles; j++) {
      ...
      SDL_Rect newRect = {(std::floor(j * mTileSize * zoom)) - camera.x,
                          (std::floor(i * mTileSize * zoom)) - camera.y,
                          std::ceil(mTileSize * zoom),
                          std::ceil(mTileSize * zoom)};
...
void RenderGuiSystem::ShowMouseLocationText(SDL_Rect &mouseBox,
                                            SDL_Rect &camera) {
  // If the mouse is on the canvas and we are creating tiles/colliders, Display
  // the mouse location
  if (!mMouseControl->MouseOutOfBounds() &&
      (mCreateTiles || mCreateColliders)) {
    // Update Grid values
    ...
    ImGui::TextColored(ImVec4(0, 255, 0, 1), "Grid [X: %d, Y: %d]", mGridX,
                       mGridY);
    ...
    // If we are in gridsnap, print the location of the TL corner of the gridbox
    // the mouse is in
    if (mGridSnap)
      ImGui::TextColored(ImVec4(0, 255, 0, 1), "Mouse [X: %d, Y: %d]",
                         mGridX * mTileSize, mGridY * mTileSize);
    else // Print the location of the mouse relative to the canvas
      ImGui::TextColored(
          ImVec4(0, 255, 0, 1), "Mouse [X: %d, Y: %d]",
          static_cast<int>(mMouseControl->GetMousePosScreen().x),
          static_cast<int>(mMouseControl->GetMousePosScreen().y));
  }
}
