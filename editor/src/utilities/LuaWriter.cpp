...
LuaWriter::LuaWriter()
    : mSeparator(','), mValueWritten(false), mMinimize(false), mNewLine(false),
      mSuppress(false), mError(false), mIndent(0) {}
...
void LuaWriter::WriteCommentLine(const std::string &comment,
                                 std::fstream &file) {
...
std::string LuaWriter::Quote(const std::string &quote) {
  ...
  for (const char ch : quote) {
    // These are special cases that must be handled correctly based on key
    // characters
    switch (ch) {
    case '\\':
      quoted.append("\\\\");
