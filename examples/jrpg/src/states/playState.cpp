...
void PlayState::SpawnTiles() {
  ...
  for (const auto &tile : loader.getMap()) {
    ...
    e.AddComponent<SpriteComponent>(tile.assetId, TILE_SRC_W, TILE_SRC_H,
...
// ─────────────────────────────────────────────────────────────────────────────
// Collider loading
// Colliders map format: collider worldX worldY scaleX scaleY colW colH offX
// offY worldX/worldY are in editor pixel coordinates (same space as tile
// worldX/worldY)
// ─────────────────────────────────────────────────────────────────────────────
...
void PlayState::LoadColliders() {
  ...
  while (f >> token) {
    if (token == "collider") {
      ...
      colliderRects_.push_back({static_cast<int>(cx), static_cast<int>(cy),
...
void PlayState::SpawnPlayer() {
  ...
  player.AddComponent<TransformComponent>(glm::vec2(100.0f, 280.0f),
                                          glm::vec2(1.0f, 1.0f), 0.0);
  // Start facing down (idle-down = frame 1, srcX = 32)
  player.AddComponent<SpriteComponent>("NPC-Sprite-Sheet", PLAYER_FRAME_W,
                                       PLAYER_FRAME_H, CHARACTER_Z, false,
                                       1 * PLAYER_FRAME_W, 0);
...
void PlayState::SpawnNPC(float x, float y, const std::string &name,
  ...
  npc.AddComponent<TransformComponent>(glm::vec2(x, y), glm::vec2(1.0f, 1.0f),
                                       0.0);
  npc.AddComponent<SpriteComponent>("NPC-Sprite-Sheet", PLAYER_FRAME_W,
                                    PLAYER_FRAME_H, CHARACTER_Z, false,
                                    idleFrame * PLAYER_FRAME_W, 0);
...
bool PlayState::CollidesWithLevel(const SDL_Rect &r) const {
  for (const auto &c : colliderRects_) {
    if (r.x < c.x + c.w && r.x + r.w > c.x && r.y < c.y + c.h &&
        r.y + r.h > c.y)
      return true;
  }
  return false;
...
void PlayState::UpdatePlayer(float dt) {
  ...
  if (keyUp_) {
    vy = -pc.moveSpeed;
    pc.facing = Direction::Up;
  }
  if (keyDown_) {
    vy = pc.moveSpeed;
    pc.facing = Direction::Down;
  }
  if (keyLeft_) {
    vx = -pc.moveSpeed;
    pc.facing = Direction::Left;
  }
  if (keyRight_) {
    vx = pc.moveSpeed;
    pc.facing = Direction::Right;
  }
  ...
  transform.position.x =
      std::max(0.0f, std::min(transform.position.x, LEVEL_W - PLAYER_FRAME_W));
  transform.position.y =
      std::max(0.0f, std::min(transform.position.y, LEVEL_H - PLAYER_FRAME_H));
...
int PlayState::PlayerSrcX(const PlayerComponent &pc) const {
  if (!pc.isMoving) {
    switch (pc.facing) {
    case Direction::Up:
      return 0 * PLAYER_FRAME_W;
    case Direction::Down:
      return 1 * PLAYER_FRAME_W;
    case Direction::Left:
      return 2 * PLAYER_FRAME_W;
    case Direction::Right:
      return 3 * PLAYER_FRAME_W;
    }
  }
  int base;
  switch (pc.facing) {
...
void PlayState::CheckNpcInteraction() {
  ...
  auto &pt =
      registry_.GetEntityByTag("player").GetComponent<TransformComponent>();
  glm::vec2 playerCenter = {pt.position.x + PLAYER_FRAME_W / 2.0f,
  ...
  if (registry_.DoesGroupExist("npcs")) {
    for (auto &npc : registry_.GetEntitiesByGroup("npcs")) {
      ...
      glm::vec2 npcCenter = {nt.position.x + PLAYER_FRAME_W / 2.0f,
...
void PlayState::UpdateDialogue(float dt) {
  ...
  if (!dialogue_.complete) {
    ...
    while (dialogue_.typeTimer >= dialogue_.typeInterval) {
      ...
      if (dialogue_.visibleChars <
          static_cast<int>(dialogue_.fullText.size())) {
...
void PlayState::render() {
  SDL_SetRenderDrawColor(renderer_, 34, 85, 34,
                         255); // dark green (grass fallback)
...
void PlayState::RenderWorld() {
  // Collect all entities with Sprite + Transform, sort by zIndex, render with
  // camera offset
  ...
  std::sort(entities.begin(), entities.end(),
            [](const Entity &a, const Entity &b) {
              ...
              const float fa =
                  a.GetComponent<TransformComponent>().position.y + sa.height;
              const float fb =
                  b.GetComponent<TransformComponent>().position.y + sb.height;
              return fa < fb;
            });
  ...
  for (auto &e : entities) {
    ...
    SDL_RenderCopyEx(renderer_, tex, &srcRect, &dstRect, t.rotation, nullptr,
                     s.flip);
  }
  ...
  if (isDebugging_) {
    ...
    for (const auto &r : colliderRects_) {
      SDL_Rect dr = {r.x - static_cast<int>(camera_.x),
                     r.y - static_cast<int>(camera_.y), r.w, r.h};
...
void PlayState::RenderDialogueBox() {
  ...
  // Speaker name (gold)
  DrawText(dialogue_.speakerName, 10 + margin, boxY + 10, {255, 220, 0, 255});
  ...
  if (dialogue_.complete) {
    DrawText("[ Press E / Space ]", windowWidth_ - 190, boxY + boxH - 24,
             {200, 200, 200, 255});
  }
