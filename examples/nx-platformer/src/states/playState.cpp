...
void PlayState::LoadAssets() {
    std::string tilesetPath = std::string(ASSET_ROOT) + "tilemaps/16x16-platformer.png";
    std::string playerPath  = std::string(ASSET_ROOT) + "gfx/player.png";
    ...
    assetStore_->AddTexture(renderer_, "player",           playerPath.c_str());
...
void PlayState::processInput() {
    ...
    if (kDown & HidNpadButton_Plus)  { isRunning_ = false; return; }

    moveLeft_  = (kHeld & HidNpadButton_Left)  || (kHeld & HidNpadButton_StickLLeft);
    moveRight_ = (kHeld & HidNpadButton_Right) || (kHeld & HidNpadButton_StickLRight);
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_SPACE:
            case SDLK_UP:
            case SDLK_w:      jumpPress_ = true;  break;
            default: break;
            }
            break;
        case SDL_KEYUP:
...
void PlayState::ResolvePlayer(float dt) {
    ...
    transform.position.x = std::max(0.0f, std::min(transform.position.x, levelWidth - pw));
