...
PlayState::PlayState(SDL_Renderer *renderer, int windowWidth, int windowHeight,
  ...
  assetStore_->AddTexture(renderer_, "block_ghost",
                          "./assets/gfx/block_ghost.png");
...
int PlayState::ClearLines() {
  ...
  for (int r = BOARD_H - 1; r >= 0;) {
    ...
    if (full) {
      ...
      for (auto &entity :
           registry_.GetSystem<RenderSystem>().GetSystemEntities()) {
        if (!entity.HasComponent<TetrisCellComponent>())
          continue;
        auto &cell = entity.GetComponent<TetrisCellComponent>();
        if (!cell.isActive && cell.boardRow == r)
          entity.Kill();
      }
...
void PlayState::processInput() {
  ...
  while (SDL_PollEvent(&e)) {
    ...
    if (e.type == SDL_KEYDOWN) {
      ...
      case SDLK_UP: {
        ...
        for (int i = 0; i < 4; i++) {
          auto &cell = activeEntities_[i].GetComponent<TetrisCellComponent>();
          cell.boardRow =
              current_.y + SHAPES[current_.type][current_.rot][i][0];
          cell.boardCol =
              current_.x + SHAPES[current_.type][current_.rot][i][1];
        }
        SyncGhostEntities();
        break;
      }
      ...
      case SDLK_LEFT: {
        ...
        dasDir_ = -1;
        dasStartTime_ = dasLastRepeat_ = SDL_GetTicks();
        break;
      }
      case SDLK_RIGHT: {
        Piece right = current_;
        right.x++;
        if (CanPlace(right)) {
          current_ = right;
          for (int i = 0; i < 4; i++)
    ...
    if (e.type == SDL_KEYUP) {
      if (e.key.keysym.sym == SDLK_LEFT && dasDir_ == -1)
        dasDir_ = 0;
      if (e.key.keysym.sym == SDLK_RIGHT && dasDir_ == 1)
        dasDir_ = 0;
    }
  }
}
...
void PlayState::RenderBoardBackground() {
  ...
  for (int r = 0; r <= BOARD_H; r++)
    SDL_RenderDrawLine(renderer_, boardOffX_, boardOffY_ + r * CELL,
                       boardOffX_ + BOARD_W * CELL, boardOffY_ + r * CELL);
  for (int c = 0; c <= BOARD_W; c++)
    SDL_RenderDrawLine(renderer_, boardOffX_ + c * CELL, boardOffY_,
...
void PlayState::RenderText(const std::string &text, int x, int y,
...
void PlayState::RenderPanel() {
  ...
  if (gameOver_) {
    ...
    RenderText("GAME OVER", boardOffX_ + 10,
               boardOffY_ + BOARD_H * CELL / 2 - 20, red, 30);
  }
}
...
void PlayState::render() {
  ...
  // Engine's RenderSystem draws all entities (locked cells, active piece,
  // ghosts)
