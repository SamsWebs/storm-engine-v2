...
bool PlayState::onEnter() {
    ...
    if (!font_) logger_.Err("Failed to open font: " + std::string(TTF_GetError()));
...
void PlayState::SpawnEntities() {
    ...
    rink.AddComponent<TransformComponent>(glm::vec2{0.f, 0.f}, glm::vec2{1.f, 1.f}, 0.0);
...
void PlayState::processInput() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_w: case SDLK_UP:    keyUp_    = true; break;
            case SDLK_s: case SDLK_DOWN:  keyDown_  = true; break;
            case SDLK_a: case SDLK_LEFT:  keyLeft_  = true; break;
            case SDLK_d: case SDLK_RIGHT: keyRight_ = true; break;
            case SDLK_SPACE:              keyShoot_  = true; break;
            }
            break;
        case SDL_KEYUP:
            switch (event.key.keysym.sym) {
            case SDLK_w: case SDLK_UP:    keyUp_    = false; break;
            case SDLK_s: case SDLK_DOWN:  keyDown_  = false; break;
            case SDLK_a: case SDLK_LEFT:  keyLeft_  = false; break;
            case SDLK_d: case SDLK_RIGHT: keyRight_ = false; break;
            case SDLK_SPACE:              keyShoot_  = false; break;
            }
            break;
        }
    }
...
void PlayState::UpdatePlayerMovement(double dt) {
    ...
    if (keyUp_)    t.position.y -= spd;
    if (keyDown_)  t.position.y += spd;
    if (keyLeft_)  t.position.x -= spd;
    if (keyRight_) t.position.x += spd;

    // Clamp to left half of rink (player stays on own side when not attacking)
    t.position.x = std::clamp(t.position.x,
                               (float)RINK_X,
                               (float)(RINK_R - PLAYER_SIZE));
    t.position.y = std::clamp(t.position.y,
                               (float)RINK_Y,
                               (float)(RINK_B - PLAYER_SIZE));
...
void PlayState::UpdateAI(double dt) {
    ...
    // ── AI Skater ─────────────────────────────────────────────────────────
    {
        ...
        t.position.x = std::clamp(t.position.x,
                                   (float)RINK_X,
                                   (float)(RINK_R - AI_SIZE));
        t.position.y = std::clamp(t.position.y,
                                   (float)RINK_Y,
                                   (float)(RINK_B - AI_SIZE));
    }
...
void PlayState::UpdatePuckCarry() {
    auto &puck  = puckEnt_->GetComponent<PuckComponent>();
    if (puck.ownerTag == -1) return;

    // Move puck just in front of the carrier
    glm::vec2 carrierPos;
    int       size = 0;
    if (puck.ownerTag == 0) { carrierPos = playerEnt_->GetComponent<TransformComponent>().position; size = PLAYER_SIZE; }
    if (puck.ownerTag == 1) { carrierPos = aiSkaEnt_->GetComponent<TransformComponent>().position;  size = AI_SIZE; }
    if (puck.ownerTag == 2) { carrierPos = aiGoalEnt_->GetComponent<TransformComponent>().position; size = GOALIE_SIZE; }
...
void PlayState::TryPickup(Entity &skater, int ownerTag) {
    ...
    int sz = (ownerTag == 2) ? GOALIE_SIZE : (ownerTag == 0 ? PLAYER_SIZE : AI_SIZE);
...
void PlayState::CheckGoal() {
    ...
    if (px < RINK_X && inGoalMouth) {
        ...
        puck.ownerTag = -1;
        puck.velocity = {0.f, 0.f};
        if (aiScore_ >= GOALS_TO_WIN) gameOver_ = true;
        return;
    }
    ...
    if (px + PUCK_SIZE > RINK_R && inGoalMouth) {
        ...
        puck.ownerTag = -1;
        puck.velocity = {0.f, 0.f};
        if (playerScore_ >= GOALS_TO_WIN) gameOver_ = true;
    }
}
...
void PlayState::DrawRink() {
    ...
    SDL_Rect leftGoal = {RINK_X - GOAL_DEPTH, GOAL_MOUTH_Y, GOAL_DEPTH, GOAL_MOUTH_H};
...
void PlayState::DrawHUD() {
    ...
    // Score
    std::string score = std::to_string(playerScore_) + "  —  " + std::to_string(aiScore_);
    ...
    // Controls hint
    DrawText("WASD: move   SPACE: shoot   ESC: quit", RINK_X, RINK_B + 10, {160, 160, 160, 255});
    ...
    if (gameOver_) {
        ...
        DrawText("Press ESC to exit", windowWidth_ / 2 - 70, windowHeight_ / 2 + 20, white, 20);
    }

    // Puck owner indicator
    auto &puck = puckEnt_->GetComponent<PuckComponent>();
    if (puck.ownerTag == 0)
        DrawText("* you have the puck *", RINK_X + 10, RINK_B + 28, {100, 200, 255, 255}, 18);
}
