...
public:
    ...
    void Update() {
        ...
        const Sint16 lx = SDL_GameControllerGetAxis(
            pad_, SDL_CONTROLLER_AXIS_LEFTX);
        const Sint16 ly = SDL_GameControllerGetAxis(
            pad_, SDL_CONTROLLER_AXIS_LEFTY);
    ...
    // marching orders need this, or holding the button retriggers every frame.
    bool PressedLeft() const  { return current_.left && !previous_.left; }
    bool PressedRight() const { return current_.right && !previous_.right; }
