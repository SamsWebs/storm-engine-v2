...
int FrameCount(world::Troop troop, const char *action) {
  ...
  if (std::string(action) == "idle")
    return archer ? 6 : 8;
  if (std::string(action) == "run")
    return archer ? 4 : 6;
...
BattleState::BattleState(SDL_Renderer *renderer, int windowWidth,
  ...
  attacker_ = {ga.side,   ga.troop,      attacker, ga.troops,
               ga.troops, Command::Hold, 0,        {}};
  defender_ = {gd.side,   gd.troop,      defender, gd.troops,
               gd.troops, Command::Hold, 0,        {}};
...
bool BattleState::onEnter() {
  ...
  logger_.Log(
      "BattleState entered: " + std::string(world::TroopName(attacker_.troop)) +
...
void BattleState::SpawnSoldiers(Side &s, bool facingRight) {
  ...
  for (int i = 0; i < VISIBLE_PER_SIDE; ++i) {
    ...
    e.AddComponent<SpriteComponent>(UnitAsset(s, "run"), UNIT_FRAME, UNIT_FRAME,
                                    /*zIndex=*/row + 1, false, 0, 0);
...
// -100 on the theory that it would tuck them behind the background: it does
// not. RenderSystem sorts by zIndex and then draws every entity it holds, and
// the battle's background is an SDL_RenderClear plus a band rect issued
// *before* the registry renders -- there is no backdrop entity to hide behind,
// so a -100 sprite simply drew first and stayed fully visible. Both armies
// showed all eight men no matter how few troops were left.
//
...
void BattleState::SyncSoldierCount(Side &s) {
  const int alive = (s.startTroops <= 0)
                        ? 0
                        : static_cast<int>(std::ceil(
                              VISIBLE_PER_SIDE *
                              (static_cast<float>(s.troops) / s.startTroops)));
...
// command bar was driving the enemy: pressing CHARGE applied the 1.6x
// multiplier to the army killing them.
...
void BattleState::processInput() {
  ...
  while (SDL_PollEvent(&event)) {
    ...
    switch (event.key.keysym.sym) {
    case SDLK_1:
      IssueCommand(Command::Charge);
      break;
    case SDLK_2:
      IssueCommand(Command::Hold);
      break;
    case SDLK_3:
      IssueCommand(Command::Volley);
      break;
    case SDLK_4:
      IssueCommand(Command::Retreat);
      break;
    case SDLK_ESCAPE:
      isRunning_ = false;
      return;
...
void BattleState::update() {
  ...
  if (confirmRetreat_) {
    ...
    if (gamepad_->Pressed(GamepadButton::A) ||
      ...
      ConfirmRetreat();
    } else if (gamepad_->Pressed(GamepadButton::B) ||
               gamepad_->Pressed(GamepadButton::Back)) {
      CancelRetreat();
    }
  ...
  if (gamepad_->Pressed(GamepadButton::A))
    IssueCommand(Command::Charge);
  if (gamepad_->Pressed(GamepadButton::B))
    IssueCommand(Command::Hold);
  if (gamepad_->Pressed(GamepadButton::X))
    IssueCommand(Command::Volley);
  if (gamepad_->Pressed(GamepadButton::Y))
    IssueCommand(Command::Retreat);
  ...
  if (!closed_) {
    ...
    if (closeT_ >= 1.0f) {
      closeT_ = 1.0f;
      closed_ = true;
      SetAnimation(attacker_, "attack", FrameCount(attacker_.troop, "attack"),
                   true);
      SetAnimation(defender_, "attack", FrameCount(defender_.troop, "attack"),
                   true);
    }
    ...
    auto close = [&](Side &s, bool fromLeft) {
      for (int i = 0; i < static_cast<int>(s.soldiers.size()); ++i) {
        ...
        const int col = i / kRowCount;
        const int spread = 74;
        const float open = fromLeft
                               ? (40 + col * spread)
                               : (windowWidth_ - 40 - drawn - col * spread);
        const float shut = fromLeft ? (centre - drawn - 10 - col * 56)
                                    : (centre + 10 + col * 56);
...
void BattleState::RenderHud() {
  ...
  auto sideBar = [&](const Side &s, bool left) {
    ...
    const SDL_Color colour =
        (s.side == world::Owner::Blue) ? ui::BlueColour() : ui::RedColour();
  ...
  const Side &player =
      (attacker_.side == world::Owner::Blue) ? attacker_ : defender_;
  for (int i = 0; i < n; ++i) {
    SDL_Texture *tex = assetStore_->GetTexture(ids[i]);
    const int cx = startX + i * slotW + slotW / 2;
    const bool active = !over_ && static_cast<int>(player.command) == i &&
                        !Expired(SDL_GetTicks(), player.commandUntil);
    if (active) {
      ui::DrawPanel(renderer_, cx - slotW / 2 + 8, y - 10, slotW - 16, 52, 200);
    }
...
void BattleState::RenderRetreatPrompt() {
  ...
  ui::DrawTextureCentred(renderer_, assetStore_->GetTexture("confirm_retreat"),
                         cx, y + 22, 1.0f);
  ui::DrawTextureCentred(renderer_, assetStore_->GetTexture("confirm_body"), cx,
                         y + 86, 1.0f);
  ui::DrawTextureCentred(renderer_, assetStore_->GetTexture("confirm_hint"), cx,
                         y + 136, 1.0f);
}
