...
const char *CastleAsset(world::Owner o) {
  switch (o) {
  case world::Owner::Blue:
    return "castle_blue";
  case world::Owner::Red:
    return "castle_red";
  default:
    return "castle_neutral";
  }
}
...
std::string RunAsset(world::Owner side, world::Troop troop) {
  ...
  const std::string kind =
      (troop == world::Troop::Archer) ? "archer" : "warrior";
...
void OverworldState::SpawnTerrain() {
  ...
  for (int y = 0; y < MAP_H; ++y) {
    for (int x = 0; x < MAP_W; ++x) {
      ...
      water.AddComponent<TransformComponent>(glm::vec2(x * TILE, y * TILE),
                                             glm::vec2(1.0f, 1.0f), 0.0);
      water.AddComponent<SpriteComponent>("water", 64, 64, kZWater, false, 0,
                                          0);
    }
  }
  ...
  for (const auto &tile : map) {
    ...
    e.AddComponent<TransformComponent>(
        ...
        glm::vec2(1.0f, 1.0f), 0.0);
    e.AddComponent<SpriteComponent>("tilemap", TILE, TILE, kZGround, false,
...
void OverworldState::SpawnDecorations() {
  ...
  for (int attempt = 0; attempt < 400 && placed < DECO_COUNT; ++attempt) {
    ...
    for (int i = 0; i < world::kCastleCount && !blocked; ++i) {
      ...
      for (int j = i + 1; j < world::kCastleCount && !blocked; ++j) {
        if (campaign_->Adjacent(i, j) &&
            DistanceToSegment(centre, centres[i], centres[j]) < kClearRoad) {
          blocked = true;
        }
      }
    }
    ...
    Entity e = registry_.CreateEntity();
    e.AddComponent<TransformComponent>(centre - glm::vec2(drawn / 2.0f),
                                       glm::vec2(k.scale, k.scale), 0.0);
    e.AddComponent<SpriteComponent>(k.asset, k.frame, k.frame, k.zIndex, false,
                                    0, 0);
    if (k.frames > 1) {
...
void OverworldState::SpawnCastles() {
  for (int i = 0; i < world::kCastleCount; ++i) {
    ...
    e.AddComponent<SpriteComponent>(CastleAsset(campaign_->castles[i].owner),
                                    kCastleW, kCastleH, kZCastle, false, 0, 0);
    castleEntities_[i] = e;
  }
}
...
void OverworldState::SyncArmyMarkers() {
  for (int i = 0; i < world::kGeneralCount; ++i) {
    ...
    if (!armyEntities_[i].has_value()) {
      ...
      e.AddComponent<SpriteComponent>(RunAsset(g.side, g.troop), kUnitFrame,
                                      kUnitFrame, kZArmy, false, 0, 0);
...
void OverworldState::CycleSelection(int delta) {
  ...
  int at = (it == pickable.end()) ? 0 : static_cast<int>(it - pickable.begin());
...
void OverworldState::StartMarch(int general, int destination) {
  ...
  if (!g.alive || g.toCastle >= 0 ||
      !campaign_->Adjacent(g.atCastle, destination)) {
    return;
  }
...
void OverworldState::RunEnemyAI() {
  for (int i = 0; i < world::kGeneralCount; ++i) {
    ...
    for (int c = 0; c < world::kCastleCount && best < 0; ++c) {
      if (!campaign_->Adjacent(g.atCastle, c))
        continue;
      if (campaign_->castles[c].owner == world::Owner::Red)
        continue;
      if (claimed(c))
        continue;
      best = c;
...
void OverworldState::update() {
  ...
  if (gamepad_->Pressed(GamepadButton::LeftShoulder) ||
      gamepad_->Pressed(GamepadButton::Left)) {
    CycleSelection(-1);
  }
  if (gamepad_->Pressed(GamepadButton::RightShoulder) ||
      gamepad_->Pressed(GamepadButton::Right)) {
    CycleSelection(1);
  }
...
void OverworldState::RenderHud() {
  ...
  if (selected_ >= 0) {
    ...
    if (!destinations_.empty()) {
      ...
      SDL_Rect ring{static_cast<int>(p.x) + 18, static_cast<int>(p.y) + 12,
