...
namespace {
    ...
    struct Rendezvous {
        ...
        bool Connect() {
            return PumpUntil(server, client, kPumpDeadlineMs, [this]() {
...
Describe(NetLoopbackSpec) {
    ...
    It(should_exchange_reliable_messages_both_ways) {
        ...
        r.client.SetOnChunk([&](const NetChunk &chunk) {
            clientSeen = RecvInt(chunk);
        });

        int32_t toClient = 12345;
        Assert::That(r.server.Send(r.clientId, &toClient, sizeof(toClient), true), Equals(true));
        Assert::That(PumpUntil(r.server, r.client, kPumpDeadlineMs,
                               [&]() { return clientSeen == 12345; }),
                     Equals(true));

        int32_t toServer = -6789;
        Assert::That(r.client.Send(&toServer, sizeof(toServer), true), Equals(true));
        Assert::That(PumpUntil(r.server, r.client, kPumpDeadlineMs,
    ...
    It(should_exchange_unreliable_messages) {
        ...
        r.client.SetOnChunk([&](const NetChunk &chunk) { clientSeen = RecvInt(chunk); });

        int32_t ping = 42;
        Assert::That(r.server.Send(r.clientId, &ping, sizeof(ping), false), Equals(true));
        Assert::That(PumpUntil(r.server, r.client, kPumpDeadlineMs,
