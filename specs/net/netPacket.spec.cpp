...
Describe(NetPacketSpec) {
    ...
    Describe(PacketHeader) {
        It(should_round_trip_flags_ack_chunks_and_token) {
            ...
            uint32_t token = 0;
            Assert::That(NetPacketHeaderUnpack(buf, 7, flags, ack, numChunks, token), Equals(true));
        ...
        It(should_round_trip_all_ack_values) {
            for (int ack = 0; ack < 1024; ack++) {
                ...
                uint32_t token = 0;
                Assert::That(NetPacketHeaderUnpack(buf, 7, flags, outAck, numChunks, token), Equals(true));
        ...
        It(should_reject_truncated_packets) {
            ...
            Assert::That(NetPacketHeaderUnpack(shortPacket, 6, flags, ack, numChunks, token), Equals(false));
        };
    };
