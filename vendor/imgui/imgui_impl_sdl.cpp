...
//  2020-05-25: Misc: Report a zero display-size when window is minimized, to be
//  consistent with other backends. 2020-02-20: Inputs: Fixed mapping for
//  ImGuiKey_KeyPadEnter (using SDL_SCANCODE_KP_ENTER instead of
//  SDL_SCANCODE_RETURN2). 2019-12-17: Inputs: On Wayland, use SDL_GetMouseState
//  (because there is no global mouse state). 2019-12-05: Inputs: Added support
//  for ImGuiMouseCursor_NotAllowed mouse cursor. 2019-07-21: Inputs: Added
//  mapping for ImGuiKey_KeyPadEnter. 2019-04-23: Inputs: Added support for
//  SDL_GameController (if ImGuiConfigFlags_NavEnableGamepad is set by user
//  application). 2019-03-12: Misc: Preserve DisplayFramebufferScale when main
//  window is minimized. 2018-12-21: Inputs: Workaround for Android/iOS which
//  don't seem to handle focus related calls. 2018-11-30: Misc: Setting up
//  io.BackendPlatformName so it can be displayed in the About Window.
//  2018-11-14: Changed the signature of ImGui_ImplSDL2_ProcessEvent() to take a
//  'const SDL_Event*'. 2018-08-01: Inputs: Workaround for Emscripten which
//  doesn't seem to handle focus related calls. 2018-06-29: Inputs: Added
...
static void ImGui_ImplSDL2_UpdateMousePosAndButtons() {
  ...
  if (g_Window == focused_window) {
    if (g_MouseCanUseGlobalState) {
      ...
      // retrieve that position globally. Won't use this workaround when on
      // Wayland, as there is no global mouse position.
      int wx, wy;
